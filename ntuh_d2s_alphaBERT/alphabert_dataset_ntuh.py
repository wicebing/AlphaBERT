    ...
    def __getitem__(self, index):
        ...
        if self.train:
            if random.random()>0.8:
                ...
                first_idx = random.choices(range(self.len),k=8)
...
class D2Lntuh_json_head(Dataset):
    def __init__(self,ds,tokenizer,clamp_size,train=True):
        self.len = len(ds)
        self.ds = ds
        self.tokenizer = tokenizer
    ...
    def __getitem__(self, index):
        ...
        for j in range(len(src_split)):
            ...
            bert_fliter_seqence_.append(torch.tensor([self.leading_token_idx],dtype=torch.long))
            bert_fliter_seqence_.append(tokens_tensors[src_split[j]].long())
