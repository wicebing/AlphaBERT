...
#        self.apply(self.init_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                position_ids=None, head_mask=None, out=None):
        output = self.bert(input_ids, attention_mask=attention_mask)
        
        if out is None:
            prediction_scores = self.cls(output[1])
#            head_score = self.pooler(output[1])
            outputs = prediction_scores, # Add hidden states and attention is they are here
        elif out == 'head':
            head_score = self.pooler(output[1])
            outputs = head_score,
        elif out == 'finehead':
