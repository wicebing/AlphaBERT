...
import alphabert_dataset_v5402 as alphabert_dataset
...
class alphabet_loaders():
    ...
    def get_data_np(self):
        ...
        elif self.bioBERTcorpus == 5:
            Gutenberg = from_Gutenberg()
            stage1_wd = np.concatenate([ntuh_wd,Gutenberg],axis=0)   
        else:
            bioBERTcorpus = from_bioBERT_corpus
        ...
        filename1ped= os.path.join(self.filepath,'finetune_test_ped.csv')
        data_test_ped = pd.read_csv(filename1ped, header=None, encoding='utf-8')
        D_L_test_ped = np.array(data_test_ped)[:,[1,3]]
    ...
    def make_loaders(self,finetune=False,head=False,headlong=False,headfirst=False,
        ...
        if finetune:
            if trvte == 'train':
                ...
                ds = data_np['D_L_val']                
            elif trvte == 'test_bing' or trvte == 'test':
                ds = data_np['D_L_test']
            elif trvte == 'test_cyy':
                ds = data_np['D_L_test_cyy'] 
            elif trvte == 'test_lin':
                ds = data_np['D_L_test_lin']                     
            elif trvte == 'test_all':
                ds = np.concatenate([data_np['D_L_test'],data_np['D_L_test_cyy'],data_np['D_L_test_lin']],axis=0)
            
            elif trvte == 'train_all':
                train = True
                ds = np.concatenate([data_np['D_L_train'],
                                     data_np['D_L_test'],
                                     data_np['D_L_test_cyy'],
                                     data_np['D_L_test_lin'],
                                     data_np['D_L_test_ped']
                                     ],axis=0)
            ...
            if head:
                ...
                if trvte == 'train':
                    ...
                    loaders = {'D2S_lin_testloader':D2S_loader,
                               'D2S_test_lin':D2S_data} 
                elif trvte == 'test_all':
                    loaders = {'D2S_all_testloader':D2S_loader,
                               'D2S_test_all':D2S_data} 
                elif trvte == 'train_all':
                    loaders = {'D2S_all_trainloader':D2S_loader,
                               'D2S_train_all':D2S_data} 
            return loaders            
#            else:
#                if trvte == 'train':
#                    train = True
...
def from_Other_corpus():
    ...
    _dat = pd.read_csv(fp1[1], encoding='ISO-8859-1',sep='\t',)
    _wd.append(np.array(_dat['TYPECONTENT']))
